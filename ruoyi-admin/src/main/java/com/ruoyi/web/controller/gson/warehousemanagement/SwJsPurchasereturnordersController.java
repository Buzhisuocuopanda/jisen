...
public class SwJsPurchasereturnordersController extends BaseController {
    ...
    public AjaxResult importSwJsGoods(MultipartFile file, boolean updateSupport) {
        try {
            ...
            log.error("【导入采购退库单】接口出现异常,参数${},异常${}$", JSON.toJSON(message), ExceptionUtils.getStackTrace(e));
...
/*    *//**
    ...
    public void importTemplate(HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            excelPaht=   RuoYiConfig.getSwdataprofile()+"采购退库单导入模板_"+time+".xlsx";
            excelPaht2 = RuoYiConfig.getSwprofile() + "模板采购退库单导入_"  + time + ".xlsx";
            FileCopyUtils.copyFile(new File(RuoYiConfig.getSwprofile()+ PathConstant.RETURN_ORDER_SCANSEWASTYY_EXCEL),new File(excelPaht2));
        ...
        String filePath = file.getAbsolutePath();*/
            ...
            FileUtils.setAttachmentResponseHeader(response, "采购退库单导入模板_.xlsx");
            FileUtils.writeBytes(excelPaht, response.getOutputStream());
        } catch (SwException e) {
            log.error("【导入采购退库单下载模板】接口出现异常,参数${}$,异常${}$", ExceptionUtils.getStackTrace(e));
